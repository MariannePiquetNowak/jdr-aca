					...
					return (
						<div className="card object" key={index}>
							<div>
								...
								<img src={st.image} width="82" height="82" alt={st.name || "Artefact"} />
								<div>
									<h4>Description</h4>
									<p>{st.description}</p>
								</div>
									...
									<div className="input_rb_ckb ammo">            
										...
										<div>
										{
											Object.entries(st.ammo)?.map(([key, value]) => (
												Object.values(st.ammo).length > 0 ? 
													<input key={key} data-index={index} data-key={key} type="checkbox" onChange={onAmmoChange} checked={value} readOnly /> 
												: null
											))
										}
										</div>
